...
function statusFor(r, gender) {
  ...
  const min =
    g === "MALE" && r.male_min != null
      ? Number(r.male_min)
      : g === "FEMALE" && r.female_min != null
        ? Number(r.female_min)
        : r.normal_min != null
          ? Number(r.normal_min)
          : null;
  const max =
    g === "MALE" && r.male_max != null
      ? Number(r.male_max)
      : g === "FEMALE" && r.female_max != null
        ? Number(r.female_max)
        : r.normal_max != null
          ? Number(r.normal_max)
          : null;
...
const COMPANY_ADDRESS =
  "6008, 6TH FLOOR, RBTIC BUILDING, IIT BOMBAY, POWAI, MUMBAI, 400076";
...
function drawPatientInfo(doc, session, startY) {
  ...
  const registeredSource =
    session.created_at || session.registered_at || session.test_date;
  ...
  const registeredDate = `${fmtDate(registeredSource)} ${fmtTime(new Date(registeredSource))}`;

  const reportedDate = `${fmtDate(reportedSource)} ${fmtTime(new Date(reportedSource))}`;
...
function drawResultsTable(
  ...
  results.forEach((r) => {
    ...
    const maxY = PAGE_H - MARGIN - footerSpace;

    if (y + RH > maxY) {
      doc.addPage();
      y = drawHeader(doc);
      y = sectionHead(doc, "LABORATORY TEST RESULTS", y) + 2;
      drawTableHeader();
    }
    ...
    const isBold =
      colorResults && status && status !== "NORMAL" && status !== "NEGATIVE";
    ...
    const testLabel = sanitize(r.test_full_name || r.test_type_name || "-");

    doc
      .fillColor(BLACK)
      .font("Helvetica")
      .fontSize(8.5)
      .text(testLabel, cx + 5, y + 8, {
        width: COLS[0] - 10,
        align: "center",
        lineBreak: false,
      });
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8.5)
      .text(bioReference(r, patientGender), cx + 5, y + 8, {
        width: COLS[3] - 10,
        align: "center",
        lineBreak: false,
      });
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8)
      .text(sanitize(methodCell(r)), cx + 5, y + 6, {
        width: COLS[4] - 10,
        align: "center",
        lineBreak: false,
      });
...
export function generateBulkReportPdf(histories, meta = {}) {
  return new Promise((resolve, reject) => {
    ...
    for (const hist of histories) {
      const h =
        typeof hist.get === "function" ? hist.get({ plain: true }) : hist;
      const p = h.patient || {};
      const eb = h.enteredBy || {};
      const results = h.results || [];
      const dateStr = fmtDate(h.test_date);
      const patientLabel = p.name ? `${p.patient_id}` : "-";
      ...
      for (const res of results) {
        ...
        const rawVal =
          res.value_text != null
            ? res.value_text
            : res.value_num != null
              ? String(res.value_num)
              : "-";
        // Qualitative results never get a unit appended
        const resDisplay =
          res.value_text != null || !unit ? rawVal : `${rawVal} ${unit}`;
...
export function generateTodayPatientReportPdf({ patient, sessions, orgName }) {
  return new Promise((resolve, reject) => {
    ...
    const sortedSessions = [...(sessions || [])].sort(
      (a, b) =>
        new Date(a.created_at || a.test_date) -
        new Date(b.created_at || b.test_date),
    );
    ...
    const todaySession = {
      ...
      // last completed/report generated session of the day
      completed_at:
        lastSession.completed_at ||
        lastSession.updated_at ||
        lastSession.test_date,

      test_date: firstSession.test_date || new Date(),
    };
    ...
    y = drawResultsTable(doc, allResults, y, patient.gender, true);
